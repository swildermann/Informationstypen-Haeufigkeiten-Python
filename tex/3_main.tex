...
Während die Einheiten der Kategorien 1,2, und 4 eine kleine Textlänge haben, sind die Module (3. Kategorie) in der Regel lang:
\begin{figure}[H]
 \begin{bchart}[step=500,max=3400,scale=1.4]
  \bcbar[text=Mitglieder]{1076}
  \bcbar[text=Felder]{619}
  \bcbar[text=Klassen]{1233}
  \bcbar[text=Module]{3385}
 \end{bchart}
\end{figure}
Die Häufigkeiten der unterschiedlichen Einheiten verteilen sich wie folgt: 
\begin{figure}[H]
 \begin{bchart}[step=300,max=2500,scale=1.4]
  \bcbar[value=exception 240]{240}
  \bcbar[text=class]{614}
  \bcbar[text=attribute]{748}
  \bcbar[text=method]{2568}
  \bcbar[text=data]{762}
  \bcbar[text=function]{1802}
  \bcbar[text=staticmethod]{1595}
...
Um für alle Kategorien ein aussagekräftiges Ergebnis erzielen zu können, werden die Stichproben unter Vorgabe von Konfidenzintervall und Fehlerspanne separat von einander pro Kategorie gezogen.  
Die minimale Anzahl der pro Kategorien zu ziehenden Einheiten wird mit dieser Formel berechnet \cite{Stichprobenziehung}:
\begin{displaymath}
MIN = \frac{n_0}{1+\frac{n_0-1}{Stichprobengroesse}} 
\end{displaymath}
wobei $n_0$ wie folgt berechnet wird: 
\begin{displaymath}
n_0 = \frac{Z^{2}*0.25}{e^{2}}
\end{displaymath}
Dabei ist Z die Angabe des Konfidenzintervalls als ??????????? und e die tolierte Fehlerrate. 
Bei einem Konfidenzintervall von 95\% ist Z=1.96 \cite{Stichprobenziehung}. 
Mit einer Fehlerrate von 5\% ergeben sich folgende Stichprobengroessen: \newline

-tabelle hier einfügen-

\subsection{Zeitaufwand}
Da die Gutachter im Rahmen eines Kurses an der Freien Universität Berlin mit 5 ECTS (Fußnote!) teilnehmen, soll der Aufwand so verteilt werden, dass die benötigten Punkte erreicht aber gleichzeitig nicht überschritten werden. Hierfür muss der Zeitaufwand pro Einheit geschätzt werden, so dass die Einheiten pro Gutachter festgelegt werden können. Diese Überlegungen sind auch schon in der Stichprobenziehung mit eingeflossen und haben dazu geführt, dass die Fehlerrate nicht weiter reduziert werden konnte. 
Die Zeitberechnung wurde wie folgt durchgeführt:  \newline
-tabelle hier einfügen- 
